...
contract Pari {
    ...
    function batchInvert64(uint256[64] memory arr) internal view {
        // 1) forward pass
        uint256 running = 1;
    ...
    //----------------------------------------------
    function computeVanishingPoly(
        uint256 chall
    ) internal view returns (uint256) {
        uint256 power = exp(chall, COSET_SIZE);
        // Z_H(x) = x^COSET_SIZE + ( -COSET_OFFSET^COSET_SIZE ), but simplified as a constant
        return addmod(power, MINUS_COSET_OFFSET_TO_COSET_SIZE, R);
    }

    //----------------------------------------------
    // comp_vq using batch inversion
    //----------------------------------------------
    function comp_vq(
        ...
        // 1) Build the 64 denominators = [chall + NEG_H_Gi_i], for i=0..63
        ...
        // 2) batch-invert them in place
        batchInvert64(denoms);

        // 3) Now each denoms[i] is actually the inverse of (chall + NEG_H_Gi_i).
        // Multiply by NOM_i, then multiply by input[i], accumulate in x_a
        uint256 x_a = 0;

        // We'll do each index in a small loop
        // For brevity, we define a "nominator" array in code or just a switch.
        // We'll do a big if.. Or define them as we do:
        // But let's do them in code for demonstration:
        // Because the original code does lagrange_i = (inverted[i]) * NOM_i
        // then x_a += lagrange_i * input[i]
        // We'll do that in a loop:

        // We'll define a local array of NOM too, or just do a switch.
        // For simplicity, define them in a fixed array:
        uint256[64] memory NOM = [
            NOM_0,
            NOM_1,
            NOM_2,
            NOM_3,
        ...
        for (uint256 i = 0; i < 64; i++) {
            // lagrange_i = denoms[i] * NOM[i], and then multiply by input[i], accumulate
            uint256 lag = mulmod(denoms[i], NOM[i], R);
            x_a = addmod(x_a, mulmod(lag, input[i], R), R);
        }

        // 4) We then do the usual steps: compute vanish, numerator, etc.
        // vanish = (chall^COSET_SIZE + constant)
        uint256 vanish = computeVanishingPoly(chall);
