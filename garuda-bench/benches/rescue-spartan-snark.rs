    ...
    num_bigint::BigUint: From<<G::ScalarField as PrimeField>::BigInt>,
{
    ...
    let mut gens = SNARKGens::<G>::new(num_cons, num_vars, num_inputs, num_nonzero_entries);
    let (mut comm, mut decomm) = SNARK::encode(&inst, &gens);
    for _ in 0..num_keygen_iterations {
        let start = ark_std::time::Instant::now();
        gens = SNARKGens::<G>::new(num_cons, num_vars, num_inputs, num_nonzero_entries);
...
fn main() {
    ...
    for &num_invocation in &num_invocations {
        ...
        let filename = format!(
            "{RESCUE_APPLICATION_NAME}-spartan-{variant}-snark-{}-{}t.csv",
