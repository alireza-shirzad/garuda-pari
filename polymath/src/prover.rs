...
impl<E: Pairing> Polymath<E> {
        ...
        <<E as Pairing>::G1Affine as AffineRepr>::BaseField: PrimeField,
    {
        ...
        let n = pk.vk.h_domain.size();
        let sigma = n + 3;
        let m0 = pk.vk.k_domain.size();
        ...
        debug_assert_eq!(m0, instance_assignment.len());
        debug_assert_eq!(num_witness_vars, witness_assignment.len());
        debug_assert_eq!(matrices.len(), 2);
        debug_assert_eq!(matrices[0].len().next_power_of_two(), n);
        debug_assert_eq!(matrices[1].len().next_power_of_two(), n);
        ...
        let w_u_hat = Evaluations::from_vec_and_domain(w_u, pk.vk.h_domain).interpolate();
        let w_w_hat = Evaluations::from_vec_and_domain(w_w, pk.vk.h_domain).interpolate();
        ...
        let (h, _) = (&z_u_hat * &z_u_hat - &z_w_hat).divide_by_vanishing_poly(pk.vk.h_domain);
        end_timer!(timer_h);
        ...
        let x_1 = compute_chall::<E>(&pk.vk, &instance_assignment[1..], &a, &c, None);
        end_timer!(timer_x1);
        ...
        let pi_poly =
            Evaluations::from_vec_and_domain(cs.instance_assignment, pk.vk.k_domain).interpolate();
        let pi_of_x1 = pi_poly.evaluate(&x_1);
        ...
        let a_over_y_to_gamma_poly = Self::mul_by_x_power(&z_u_hat_sparse, sigma * MINUS_GAMMA)
            + Self::mul_by_x_power(&ra_sparse_poly, sigma * (MINUS_GAMMA - MINUS_ALPHA));
        let r_poly_term_one = &Self::mul_by_x_power(&z_u_hat_sparse, sigma * MINUS_GAMMA)
            .mul(&ra_sparse_poly)
            * E::ScalarField::from(2_i32);
        let r_poly_term_two = Self::mul_by_x_power(
            &ra_sparse_poly.mul(&ra_sparse_poly),
            sigma * (MINUS_GAMMA - MINUS_ALPHA),
        );
        ...
        let c_over_y_to_gamma_poly: SparsePolynomial<E::ScalarField> =
            Self::mul_by_x_power(&w_u_hat_sparse, sigma * MINUS_ALPHA)
                + Self::mul_by_x_power(&w_w_hat_sparse, sigma * (MINUS_GAMMA - MINUS_ALPHA))
                + Self::mul_by_x_power(&h_zh_poly_sparse, sigma * (MINUS_GAMMA - MINUS_ALPHA))
        ...
        let d_over_y_to_gamma_nom_poly: DenseOrSparsePolynomial<E::ScalarField> =
            DenseOrSparsePolynomial::from(
                (a_over_y_to_gamma_poly + &c_over_y_to_gamma_poly * x_2)
                    + (-Self::mul_by_x_power(&d_right_coeff_poly, sigma * MINUS_GAMMA)),
            );
        let d_denom_poly =
        ...
        let (d_over_y_to_gamma_poly, _d_rem_poly) = d_over_y_to_gamma_nom_poly
            .divide_with_q_and_r(&d_denom_poly)
            .unwrap();
        // Compute the group element d
        let d = <E::G1 as VariableBaseMSM>::msm_unchecked(
            &pk.x_z_g1_vec,
            &d_over_y_to_gamma_poly.coeffs,
        );
