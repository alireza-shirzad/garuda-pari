use ark_relations::{
    gr1cs::{predicate::PredicateConstraintSystem, ConstraintSynthesizer, ConstraintSystemRef, Label, Matrix},
    lc, ns,
};
use ark_ff::Field;
...
impl<F: Field + core::convert::From<i8>> ConstraintSynthesizer<F> for Circuit1<F> {
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> ark_relations::utils::Result<()> {
